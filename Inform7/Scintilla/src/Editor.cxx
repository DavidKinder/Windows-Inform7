...
/*XXXXDK Indent for wrapped lines */
			wrapAddIndent = wrapVisualStartIndent * vs.spaceWidth;
/*XXXXDK Indent for wrapped lines */
...
/*XXXXDK Show only part of document */
  if (xVisibleStart > 0 || xVisibleEnd > 0) {
    ...
    if (xVisibleStart > 0 && xVisibleStart < pdoc->LinesTotal() && lineDoc < xVisibleStart)
      pos = pdoc->LineStart(xVisibleStart);
    else if (xVisibleEnd > 0 && xVisibleEnd < pdoc->LinesTotal() && lineDoc > xVisibleEnd)
      pos = pdoc->LineEnd(xVisibleEnd);
  }
/*XXXXDK Show only part of document */
	return pos;
}
...
/*XXXXDK Paint background */
  {
    PRectangle rc = rcLine;
    rc.left = 0;
    rc.right = xStart;
	  surface->FillRectangle(rc, overrideBackground ? background : vsDraw.styles[STYLE_DEFAULT].back.allocated);
  }
/*XXXXDK Paint background */
...
*/
          Point from(0, 0);
					PRectangle rcCopyArea(0, yposScreen,
/*XXXXDK Paint background */
...
/*XXXXDK Show only part of document */
  if (xVisibleStart > 0 || xVisibleEnd > 0) {
      ...
      if (xVisibleStart > 0 && lineOfPos < xVisibleStart) {
        if (mh.linesAdded < 0) {
          if (lineOfPos-mh.linesAdded > xVisibleStart)
            xVisibleStart = lineOfPos;
          else
            xVisibleStart += mh.linesAdded;
        }
        else
          xVisibleStart += mh.linesAdded;
      }
      ...
      if (xVisibleEnd > 0 && lineOfPos <= xVisibleEnd) {
        if (mh.linesAdded < 0) {
          if (lineOfPos-mh.linesAdded > xVisibleEnd)
            xVisibleEnd = lineOfPos;
          else
            xVisibleEnd += mh.linesAdded;
        }
        else
          xVisibleEnd += mh.linesAdded;
      }
      if (xVisibleEnd <= xVisibleStart)
        xVisibleEnd = 0;
	  }
    cs.SetVisibleRegion(xVisibleStart, xVisibleEnd);
  }
/*XXXXDK Show only part of document */
...
/*XXXXDK Show only part of document */
	case SCIX_VISIBLEREGION:
    if (wParam >= 0 && (lParam == 0 || lParam > wParam)) {
      xVisibleStart = wParam;
      xVisibleEnd = lParam;
      cs.SetVisibleRegion(xVisibleStart, xVisibleEnd);
		  SetScrollBars();
		  Redraw();
	...
	case SCIX_ISLINEINVISIBLEREGION:
    if (xVisibleStart > 0) {
      if (wParam < xVisibleStart)
        return 0l;
    }
    if (xVisibleEnd > 0) {
      if (wParam > xVisibleEnd)
        return 0l;
    }
    return 1l;

	case SCIX_VISIBLEREGIONSTART:
    if (xVisibleStart > 0 && xVisibleStart < pdoc->LinesTotal()) {
      return pdoc->LineStart(xVisibleStart);
    }
    return 0l;

	case SCIX_VISIBLEREGIONEND:
    if (xVisibleEnd > 0 && xVisibleEnd < pdoc->LinesTotal()) {
      return pdoc->LineEnd(xVisibleEnd);
    }
    return pdoc->Length();
/*XXXXDK Show only part of document */
