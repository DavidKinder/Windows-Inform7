...
static UINT CodePageFromCharSet(DWORD characterSet, UINT documentCodePage) {
	...
	BOOL bci = ::TranslateCharsetInfo(reinterpret_cast<DWORD*>(static_cast<ptrdiff_t>(characterSet)),
		&ci, TCI_SRCCHARSET);
...
/*XXXXDK Custom scrollbar */
  ...
  scn.lpsi = lpsi;
  scn.bRedraw = bRedraw;
	LRESULT res = ::SendMessage(::GetParent(MainHWND()), WM_NOTIFY,
...
/*XXXXDK Change pasted text */
        SCNXConvertPaste scn;
        memset(&scn, 0, sizeof(scn));
	      scn.nmhdr.code = SCNX_CONVERTPASTE;
	      scn.nmhdr.hwndFrom = MainHWND();
	      scn.nmhdr.idFrom = GetCtrlID();
        scn.utext = uptr;
        scn.ulen = bytes / 2;
...
/*XXXXDK Change pasted text */
void ScintillaWin::CopyToClipboard(const SelectionText &selectedText_) {
/*XXXXDK Change pasted text */
	if (!::OpenClipboard(MainHWND()))
		return ;
	::EmptyClipboard();
...
/*XXXXDK Change pasted text */
  SelectionText selectedText;
  selectedText.Copy(selectedText_);

  SCNXConvertCopyToClip scn;
  memset(&scn, 0, sizeof(scn));
	scn.nmhdr.code = SCNX_CONVERTCOPYTOCLIP;
	scn.nmhdr.hwndFrom = MainHWND();
	scn.nmhdr.idFrom = GetCtrlID();
  scn.text = selectedText.s;
	LRESULT res = ::SendMessage(::GetParent(MainHWND()), WM_NOTIFY,
	                            GetCtrlID(), reinterpret_cast<LPARAM>(&scn));
  if (res != 0) {
    selectedText.Set(scn.text,strlen(scn.text)+1,
                     selectedText.codePage,selectedText.characterSet,
                     selectedText.rectangular,selectedText.lineCopy);
  }
/*XXXXDK Change pasted text */
