...
void GameWindow::RunInterpreter(const char* dir, const char* file, bool glulx)
{
  ...
  m_fontSize = theApp.MeasureFont(this,theApp.GetFont(this,InformApp::FontFixedWidth));
  ...
  // the interpreter.
  Process proc = theApp.CreateProcess(dir,command,start,!(::IsDebuggerPresent()));
  if (proc.process != INVALID_HANDLE_VALUE)
  {
    m_interpreter = proc.process;
