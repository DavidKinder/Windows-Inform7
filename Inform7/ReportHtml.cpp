...
// Implementation of visitor for modifying the DOM if dark mode is not handled by the HTML
class ModifyForDarkModeVisitor : public CefStringVisitor
{
public:
  // The visitor constructor takes a reference to the frame to allow running Javascript
  ModifyForDarkModeVisitor(CefRefPtr<CefFrame> frame) : m_frame(frame)
  {
  }

  // Check if the HTML source handles dark mode and if not, run Javascript to adjust
  // the background
  void Visit(const CefString& source)
  {
    if (source.ToString().find("color-scheme: light dark;") == std::string::npos)
    {
      // Set the default background to light grey
      m_frame->ExecuteJavaScript("document.body.style.backgroundColor = 'lightgrey';","",0);
    }
  }

private:
  IMPLEMENT_REFCOUNTING(ModifyForDarkModeVisitor);
...
// Execute JavaScript to adjust HTML generated by the compiler to look okay
// in Windows' dark mode.
void ReportHtml::ModifyHTMLForDarkMode(void)
{
  CString blankPath = theApp.GetAppDir()+"\\Documentation\\windows\\Blank.html";
  if (m_url != blankPath)
  {
    CefRefPtr<CefFrame> frame = m_private->browser->GetMainFrame();
    CefRefPtr<CefStringVisitor> visitor = new ModifyForDarkModeVisitor(frame);
